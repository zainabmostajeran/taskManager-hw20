...
const InProgressPage: React.FC = () => {
  ...
  const highPriority = inProgressTodos.filter(
    (todo) => todo.priority === "high"
  );
  const mediumPriority = inProgressTodos.filter(
    (todo) => todo.priority === "medium"
  );
  const lowPriority = inProgressTodos.filter((todo) => todo.priority === "low");
    ...
    <ProtectedRoute>
      ...
      <div className="p-4">
        ...
        {inProgressTodos.length > 0 ? (
          ...
          <div>
            <h3>No in progress todos available</h3>
          </div>
        )}
      </div>
